...
export default function htmlInclude(options = {}) {
    const { extensions = ['.html', '.svg'], delimiters = ['{{', '}}'], allowAbsolutePaths = false, watch = true, } = options;
    let viteConfig = null;
    function resolveWithAlias(fileAttr) {
     ...
     */
    async function processIncludes(inputHtml, baseDir, inheritedVars, sourceFile = null) {
        ...
        while (true) {
            ...
            const includedHtml = await processIncludes(content, path.dirname(resolvedPath), mergedVars, sourceFile || resolvedPath // on garde la source initiale
            );
            const interpolated = interpolateVariables(includedHtml, mergedVars);
            const parsed = parse(interpolated);
            // Slot handling
            const slotNamedMap = new Map();
            const slotTemplates = tag.querySelectorAll('template[slot]');
            slotTemplates.forEach(tpl => {
                const name = tpl.getAttribute('slot');
                if (name) {
                    slotNamedMap.set(name, tpl.innerHTML.trim());
                }
            });
            const defaultContentToInject = tag.childNodes
                .filter(node => {
                if (node.nodeType !== 1)
                    return true;
                const element = node;
                return !(element.tagName.toLowerCase() === 'template' && element.getAttribute('slot'));
            })
            ...
            if (children.length === 1) {
                const rootEl = children[0];
                for (const [attr, val] of Object.entries(tag.attributes)) {
                    if (attr.startsWith('$') || attr.startsWith('data-$') || attr === 'file')
                        continue;
                    if (attr === 'class') {
     ...
     */
    function extractVars(attributes) {
